...
{
public:
    std::vector<Node<K, V> *> nodes; // 实际的block存储结构
    std::vector<K> keys;
...
void SkipList<K, V>::bulkload(std::vector<std::pair<std::pair<K, V>, double>> vec)
{
    ...
    for (int i = 0; i <= _max_level; i++)
        current_blocks[i] = _header->forward_blocks[i];
    ...
    std::cout << "bulk load end\nStart building PLA-model..." << std::endl;
    // traversal all blocks and build PLA-model
    for (int i = 0; i <= _skip_list_level; i++)
    {
        // first block in level i
        Block<K, V> *block = this->_header->forward_blocks[i];
        while (block)
        {
            // build pla
