...
public:
     ...
     *  - 每层每段平均含多少键
     */
    void print_index_structure() const {
        // 总层数
        size_t lvls = height();
        std::cout << "Number of levels: " << lvls << std::endl;

        // 第0层覆盖 n 个原始键
        // 第i层覆盖上一层的 segments 数量
        size_t keys_in_current_level = n;

        // 依次输出每一层信息
        for (size_t i = 0; i < lvls; i++) {
            // 当前层段数
            size_t level_segment_count = levels_offsets[i+1] - levels_offsets[i];

            // 计算平均每段包含的键数
            double avg_keys_per_segment = 0.0;
            if (level_segment_count > 0) {
                avg_keys_per_segment = double(keys_in_current_level) / double(level_segment_count);
            }

            std::cout << "Level " << i << ": "
                      << level_segment_count << " segments, "
                      << "avg keys/segment = " << avg_keys_per_segment
                      << std::endl;

            // 下一层要索引的是本层的段数（因为上层对下层的“段首 key”再做模型）
            keys_in_current_level = level_segment_count;
        }
    }
