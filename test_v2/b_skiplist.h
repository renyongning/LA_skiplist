...
int SkipList<K,V>::insert(const K key,const  V value,double p)
{
    ...
    //添加的值没有在跳表中，执行插入，插入node的同时维护block结构，如果插入到i（i>0）层,还需要对0到i-1层的block进行分割
    if(current==NULL||current->get_key()!=key)
    {
        ...
        for(int i=0;i<random_level;i++)
        {
            ...
            if(i<random_level-1) //非最高层级,节点x插入Block中,同时分割Block,即由[left,...,right]变为[left,...,before_x] [x,...,right]
            { 
                inserted_node->forward_blocks[i]=insert_and_divide(update_blocks[i],inserted_node);
            } 
            else if(i==random_level-1) // 在最高层级（random_level-1），将节点插入到 nodes 中,不用分裂
            {
...
void SkipList<K,V>::delete_element(K key)
{
    ...
    //实现删除node,如果删除的节点x层级i>0,还需要同时对0到i-1层的Block进行合并
    if(current!=NULL&&current->get_key()==key)
    {
        for(int i=0;i<=_skip_list_level;i++) {
...
//search为针对LA_skiplist的原始版本,没有利用Block,效率低
...
//search2为利用了Block的版本,同时加入Block内进行二分搜索,在Block内节点更多时效率高
template<typename K, typename V>
